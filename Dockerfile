...
RUN mkdir  $HOME/.vnc
RUN echo 's325z' | vncpasswd -f > $HOME/.vnc/passwd
RUN echo '/bin/env  MOZ_FAKE_NO_SANDBOX=1  dbus-launch xfce4-session'  > $HOME/.vnc/xstartup
RUN chmod 600 $HOME/.vnc/passwd
RUN chmod 755 $HOME/.vnc/xstartup
RUN echo 'whoami ' >>/s325z.sh
RUN echo 'cd ' >>/s325z.sh
RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/s325z.sh
RUN echo 'cd /noVNC-1.2.0' >>/s325z.sh
RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/s325z.sh
RUN chmod 755 /s325z.sh
EXPOSE 8900
CMD  /s325z.sh
